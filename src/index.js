...
class App {
  ...
  updateTimer(time = moment()) {
    ...
    if (rest <= 0) {
      ...
      if (this.onWork) endAt = moment(time).add(this.workLength, 'minutes'); // ここまでは作業中の話をしていますから、作業中の場合のendAtに、作業時間workLengthの代入をする必要がありそうです
      if (!this.onWork) {  // 状況は作業中ではないときです
